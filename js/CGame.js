function CGame(oData, iLevel) {
    ...
    this._init = function () {
        ...
        console.log("Initial game over:", _bGameOver); // Debug
            ...
            if (window.errorLogger) {
                window.errorLogger.showErrorToUser('Game could not be started. Please refresh the page.');
            }
    ...
    this.gameOver = function ()
    {
        ...
        console.log("🎯 GameOver: this._submitToLeaderboard exists:", typeof this._submitToLeaderboard);
        console.log("🔥 GameOver DIRECT TEST - window.s_oLeaderboardManager:", typeof window.s_oLeaderboardManager);
        
        try {
            // Calculate game time, ensure it's at least 1 second
            var gameTime = Math.max(1, Math.floor((new Date().getTime() - _iGameStartTime) / 1000));
            ...
            console.log("🔥 GameOver: About to call _submitToLeaderboard with:", gameData);
            console.log("🔥 GameOver: Game time calculated:", gameTime, "seconds");
            this._submitToLeaderboard(gameData);
        } catch (submitError) {
            console.error("❌ GameOver: Error calling _submitToLeaderboard:", submitError);
...
this.setupZOrder = function() {
    ...
    console.log("Z order setup complete: Parallax -> First Platform -> Other Platforms -> River -> Shark -> Character (front)");
};
...
// Save game result to JSON leaderboard and update social features
    ...
    this._createGameOverButtons = function() {
        ...
        restartBtn.innerHTML = '<img src="sprites/but_restart.png" style="width: 70px; height: auto;">';
        ...
        homeBtn.innerHTML = '<img src="sprites/but_home.png" style="width: 70px; height: auto;">';
    ...
    this.startGameplay = async function() {
        ...
        if (_oObstacleManager) {
            _oObstacleManager.updateMultipliers(_fBetAmount, _sDifficulty);
            console.log("🎯 Platform multipliers updated for difficulty:", _sDifficulty);
        }
        ...
        try {
            ...
            if (contract && address) {
                ...
                if (hasActive) {
                    console.log("⚠️ Player has active round - checking resume possibility...");
                    ...
                    const userChoice = await this._showRecoveryDialog(resumeInfo.canResume, resumeInfo.reason);
                    ...
                    if (userChoice === 'resume') {
                        ...
                        try {
                            ...
                            if (!stillHasActive) {
                                this._showGameNotification("❌ Round Not Found", "Active round no longer exists. Please start a new game.", "error");
                                return;
                            }
                            ...
                            if (!roundInfo.hasRound) {
                                this._showGameNotification("❌ Round Not Found", "Round information not available. Please start a new game.", "error");
                                return;
                            }
                            
                            this._showGameNotification("🎮 Resume Feature", "Resume functionality coming soon! Using recovery for now.", "warning");
                            
                            // Fallback to recovery for now
                            const recoveryResult = await window.walletManager.recoverStuckRound();
                            if (recoveryResult.success) {
                                this._showGameNotification("✅ Funds Recovered", "Your round was ended and funds recovered.", "success");
                            }
                            return;
                            
                        } catch (resumeError) {
                            console.error("❌ Resume error:", resumeError);
                            this._showGameNotification("❌ Resume Failed", "Could not resume game. Try recovery instead.", "error");
                            return;
                        }
                        ...
                        try {
                            ...
                            const stillHasActive = await contract.hasActiveRound(address);
                            if (!stillHasActive) {
                                this._showGameNotification("❌ Round Not Found", "Active round no longer exists. Starting new game.", "warning");
                                // Continue with normal game start
                            } else {
                                const recoveryResult = await window.walletManager.recoverStuckRound();
                            
                            if (recoveryResult.success) {
                                console.log("✅ Immediate recovery successful:", recoveryResult.txHash);
                                ...
                                console.error("❌ Immediate recovery failed:", recoveryResult.error);
                                ...
                                if (recoveryResult.needsTimeout) {
                                    ...
                                    this._showGameNotification("❌ Recovery Failed", recoveryResult.error, "error");
                                }
                                return;
                            }
                            }
                            ...
                            this._showGameNotification("❌ Recovery Error", recoveryError.message + " Please try again later.", "error");
                            return;
                        }
                        
                    } else {
                        console.log("❌ User cancelled");
                        this._showGameNotification("❌ Cancelled", "Please complete or recover your current round to start a new game.", "warning");
                        return;
                    }
                }
            }
        } catch (activeCheckError) {
            console.warn("⚠️ Could not check for active round:", activeCheckError.message);
            // Continue with game start anyway
        }
        
        // Balance kontrolü
        if (!window.walletManager.canPlaceBet(_fBetAmount)) {
            console.log("❌ Insufficient balance:", window.walletManager.getBalance(), "needed:", _fBetAmount);
            if (window.errorLogger) {
                window.errorLogger.showErrorToUser('Insufficient MON balance. Need ' + _fBetAmount + ' MON to play.');
            }
            return;
        }
        ...
        if (!roundResult.success) {
            ...
            if (window.errorLogger) {
                window.errorLogger.showErrorToUser('Failed to start game: ' + roundResult.error);
            }
            return;
        }
    ...
    this.cashout = async function() {
        try {
            ...
            if (!_bFirstJumpCompleted) {
                ...
                if (window.errorLogger) {
                    window.errorLogger.showErrorToUser("You must complete at least one jump before cashing out!");
                }
                return;
            }
            ...
            if (window.walletManager && window.walletManager.isGameActive()) {
                ...
                if (!contractResult.success) {
                    ...
                    if (window.errorLogger) {
                        window.errorLogger.showErrorToUser('Cashout transaction failed: ' + contractResult.error);
                    }
            ...
            if (_iSuccessfulJumps === 0) {
                netWinnings = 0;
                console.log("🚫 No jumps made - counting as refund only, net winnings = 0");
            }
        ...
        console.log("  - Total payout:", winnings.toFixed(2), "MON (bet + profit)");
        console.log("  - Net profit:", netWinnings.toFixed(2), "MON (for leaderboard)");
        ...
        console.log("🎯 this._submitToLeaderboard exists:", typeof this._submitToLeaderboard);
        
        // IMMEDIATE TEST: Call directly without try-catch to see exact error
        console.log("🔥 DIRECT TEST - window.s_oLeaderboardManager:", typeof window.s_oLeaderboardManager);
        console.log("🔥 DIRECT TEST - window.s_oLeaderboardManager:", window.s_oLeaderboardManager);
        ...
        try {
            ...
            console.log("🔥 DIRECT TEST - About to call _submitToLeaderboard with:", gameData);
            ...
            console.log("🔥 FINAL VALUES - Platforms:", finalPlatforms, "Multiplier:", finalMultiplier, "Bet:", finalBetAmount);
            this._submitToLeaderboard(gameData);
        } catch (submitError) {
    ...
    this.cashoutWithMultiplier = async function(specificMultiplier) {
        ...
        console.log("🎉 MANUAL CASHOUT - Starting contract cashout with multiplier:", specificMultiplier.toFixed(2));
        ...
        try {
            ...
            if (window.walletManager && window.walletManager.cashOut) {
                ...
                if (result.success) {
                    ...
                    console.log("   🏆 Player successfully cashed out from celebration platform");
                    
                    // Calculate expected winnings (for display)
                    var expectedWinnings = Math.min(_fBetAmount * specificMultiplier, 100); // Max 100 MON
                    ...
                    console.log("🎉 Real contract winnings automatically processed by WalletManager");
    ...
    this._fallbackCashout = function(specificMultiplier) {
        ...
        if (window.walletManager && window.walletManager.clearGameState) {
            window.walletManager.clearGameState();
            console.log("🧹 Game state cleared in wallet manager on fallback cashout");
        }
        
        var winnings = Math.min(_fBetAmount * specificMultiplier, 100); // Max 100 MON
        
        console.log("Fallback cashout! Multiplier:", specificMultiplier.toFixed(2), "Winnings:", winnings.toFixed(2), "MON");
        ...
        if (window.walletManager && window.walletManager.processWin) {
            console.log("🎉 Adding fallback celebration platform winnings to profit:", winnings.toFixed(4), "MON");
    ...
    this.tapScreen = async function () {
        ...
        console.log("🔍 JUMP DEBUG - WalletManager exists:", !!window.walletManager);
        console.log("🔍 JUMP DEBUG - Game active:", window.walletManager ? window.walletManager.isGameActive() : "N/A");
        console.log("🔍 JUMP DEBUG - Connected:", window.walletManager ? window.walletManager.isConnected() : "N/A");
        ...
        if (window.walletManager && window.walletManager.isGameActive()) {
            ...
            const currentSuccessfulJumps = this.getSuccessfulJumps ? this.getSuccessfulJumps() : 0;
            const currentJumpsLeft = this.getJumpsLeft();
            console.log("🔍 Celebration check - Successful jumps:", currentSuccessfulJumps);
            ...
            if (isCelebrationJump) {
                console.log("🎉 CELEBRATION JUMP - No transaction needed, direct celebration!");
                console.log("🎉 Player completed 7 platforms, now jumping to celebration platform!");
            ...
            console.log("🔧 NORMAL JUMP - Platform", currentSuccessfulJumps + 1, "- Transaction needed");
            ...
            try {
                ...
                if (result.success) {
                    ...
                    console.log("🎯 REAL Contract result from transaction:", contractResult, "Round ended:", result.roundEnded);
                    ...
                    console.log("💀 JUMP TRANSACTION FAILED - Contract round may have ended");
                    this.gameOver();
                    return;
                }
            } catch (error) {
                ...
                console.log("💀 JUMP TRANSACTION ERROR - Contract round may have ended");
                this.gameOver();
                return;
            }
        }
    ...
    this.updateUIAfterLanding = function(platformIndex, isCelebrationPlatform) {
        ...
        if (platformIndex === 7 || isCelebrationPlatform) {
            ...
            console.log("🎉 FINAL CELEBRATION - Setting game over state to prevent further actions");
        ...
        var currentMultipliers = (_sDifficulty === "hard") ? hardMultipliers : easyMultipliers;
        // Validate platform index
        if (platformIndex < 0 || platformIndex >= currentMultipliers.length) {
            console.error("❌ Invalid platform index for multiplier:", platformIndex, "Array length:", currentMultipliers.length);
            platformIndex = Math.max(0, Math.min(platformIndex, currentMultipliers.length - 1));
            console.log("🔧 Corrected platform index to:", platformIndex);
        }
        
        var platformMultiplier = currentMultipliers[platformIndex];
        
        console.log("🔍 Multiplier debug - Difficulty:", _sDifficulty, "Platform index:", platformIndex, "Selected array:", _sDifficulty === "hard" ? "HARD" : "EASY");
        console.log("🔍 Available multipliers:", currentMultipliers);
        console.log("🔍 Selected multiplier for index", platformIndex, ":", platformMultiplier);
        ...
        if (_oCharacterManager && _oCharacterManager.updateWinningsDisplay) {
            console.log("🔄 Updating character winnings with new multiplier:", _fCurrentMultiplier);
            _oCharacterManager.updateWinningsDisplay();
        }
        
        // Update side panels with live stats
        if (window.sidePanels) {
            window.sidePanels.updateLiveStats(_fCurrentMultiplier, _iSuccessfulJumps, _fBetAmount);
        }
    ...
    this.update = function () {
        ...
        if (_oCharacterManager && _oCharacterManager.getX) {
            ...
            if (charX !== _lastParallaxX && (!_oCharacterManager.onGround() || _oCharacterManager.isCelebrationActive())) {
        ...
        return new Promise((resolve) => {
            ...
            dialog.innerHTML = `
                <div style="color: #fff; margin-bottom: 20px;">
                    ...
                    <p style="margin: 0 0 15px 0; font-size: 14px; color: ${canResume ? '#81c784' : '#ffb74d'};">
                        ${resumeReason}
                    </p>
                ...
                <div style="display: flex; flex-direction: column; gap: 12px; align-items: center;">
                    <button id="resume-btn" style="
                        background: ${canResume ? 'linear-gradient(135deg, #2196f3, #1976d2)' : 'linear-gradient(135deg, #757575, #616161)'};
                        color: white;
                        border: none;
                        padding: 12px 24px;
                        border-radius: 8px;
    ...
    this._setupContractEventHandlers = function() {
        ...
        window.addEventListener('contractRoundEnded', function(event) {
            ...
            if (event.detail.failed) {
                ...
                console.log("🎉 Contract round ended successfully - winnings:", event.detail.winAmount);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    return { canResume: false, reason: "Round information not available" };
                }
            }
            ...
            console.log(`🔍 Resume check result: ${canResume ? 'CAN RESUME' : 'CANNOT RESUME'} - ${reason}`);
            ...
            return { canResume: false, reason: "Error checking round status: " + error.message };
        }
    };
    ...
    this._submitToLeaderboard = function(gameData) {
        ...
        console.log("📊 DEBUG - gameData object:", JSON.stringify(gameData, null, 2));
        
        try {
            // Force use window.s_oLeaderboardManager to avoid scope issues
            var leaderboardManager = window.s_oLeaderboardManager;
            
            console.log("📊 DEBUG - window.s_oLeaderboardManager exists:", typeof leaderboardManager);
            console.log("📊 DEBUG - leaderboardManager object:", leaderboardManager);
            
            if (leaderboardManager && typeof leaderboardManager.submitGameResult === 'function') {
                console.log("📊 Submitting game result to leaderboard:", gameData);
                console.log("📊 WINNINGS VALUE IN GAMEDATA:", gameData.winnings);
                
                leaderboardManager.submitGameResult(gameData)
                    .then(function(result) {
                        console.log("✅ Game result submitted to leaderboard successfully:", result);
                        console.log("✅ RESULT DATA:", JSON.stringify(result, null, 2));
                    })
                    .catch(function(error) {
                        console.error("❌ Failed to submit game result to leaderboard:", error);
                        console.error("❌ ERROR DETAILS:", error.message);
                        console.error("❌ ERROR STACK:", error.stack);
                        // Don't show error to user, just log it
                    });
            } else {
                console.log("⚠️ Leaderboard manager not available, skipping submission");
                console.log("⚠️ DEBUG - leaderboardManager:", leaderboardManager);
                console.log("⚠️ DEBUG - submitGameResult method:", typeof (leaderboardManager && leaderboardManager.submitGameResult));
                console.log("⚠️ DEBUG - Available methods:", leaderboardManager ? Object.keys(leaderboardManager) : 'null');
            }
        } catch (error) {
