...
function playSound(szSound,iVolume,bLoop){
    console.log("🔊 playSound called:", szSound, "Volume:", iVolume, "Loop:", bLoop);
    console.log("🔊 Sound available:", !!s_aSounds[szSound], "Mobile disabled:", DISABLE_SOUND_MOBILE, "Is mobile:", s_bMobile);
    
    // AudioContext unlock check for Chrome's autoplay policy - Updated for Howler.js v2.2.3
    if (typeof Howler !== 'undefined' && Howler.ctx && Howler.ctx.state === 'suspended') {
        console.log("🔊 AudioContext suspended - attempting to resume...");
        Howler.ctx.resume().then(function() {
            console.log("✅ AudioContext resumed for playback");
            // Don't recursively call - just continue with playback
        }).catch(function(error) {
            console.error("❌ Failed to resume AudioContext:", error);
            return null;
        });
        // Continue with playback after resume attempt
    }
    
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        if (s_aSounds[szSound]) {
            try {
                // Check if Howler is globally muted - Updated for v2.2.3
                if (typeof Howler !== 'undefined' && Howler._muted) {
                    console.log("🔇 Howler is globally muted - unmuting for sound:", szSound);
                    Howler.mute(false);
                }
                
                var soundInstance = s_aSounds[szSound].play();
                
                // Set volume and loop after play() call
                if (soundInstance) {
                    s_aSounds[szSound].volume(iVolume);
                    s_aSounds[szSound].loop(bLoop);
                    console.log("✅ Sound played successfully:", szSound, "Instance:", soundInstance);
                } else {
                    console.error("❌ Sound play() returned null/undefined:", szSound);
                }
                
                return soundInstance;
            } catch (error) {
                console.error("❌ Error playing sound:", szSound, error);
                return null;
            }
        } else {
            console.error("❌ Sound not found:", szSound);
            console.log("Available sounds:", Object.keys(s_aSounds || {}));
        }
    } else {
        console.log("🔇 Sound disabled for mobile");
    }
    return null;
}
